...
1. prepare 失敗時の意味論: 全 Plan 破棄（イベント丸ごと無効）を推奨。
   部分 commit は原子性の放棄なので選ばない。
2. profit trial（rollback 付き試行）の座席: root の裁定内サブプロトコルか、
   engine のサービスか。推奨は root（裁定の一種であるため）。
3. capture の位相所有: observe_window は cadence の一部として根が持つか。
   推奨は根（cadence 根所有の裁定を貫徹する）。
