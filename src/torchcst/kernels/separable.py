...
class Separable(Kernel):
    ...
    def factors(
        ...
        split = self.input_profile.parameter_dim(input_chart)
        phi_input = self.input_profile.evaluate(input_chart, p[:, :split])
        phi_output = self.output_profile.evaluate(output_chart, p[:, split:])
