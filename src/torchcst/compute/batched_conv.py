...
a chain of small ops -- two factor-matrix evaluations (broadcast subtract,
square, reduce, then the factor profile) and one contraction -- all sized by
...
* Gradients to ``s``, ``t``, and ``sigma`` are *not* guaranteed
  ...
  factor matrix; ``s``/``t`` over the *other* side's feature count), so its
...
def _validate_batch_group(
    ...
    for layer in layers:
        ...
        if (
            type(layer.factor_in) is not type(ref.factor_in)
            or type(layer.factor_out) is not type(ref.factor_out)
        ):
            raise ValueError(
                "batched_conv_dense_weights requires every layer to share "
                "factor_in/factor_out family"
            )
...
def batched_conv_dense_weights(layers: Sequence[CSTConv2d]) -> list[Tensor]:
    ...
    ``s``/``t``/``w``, chart ``mu``, factor ``sigma``, and gates flow to the
    ...
    - ``factor_in`` family and ``factor_out`` family (their ``sigma``
    ...
    sigma_in_stack = torch.stack(
        [layer.factor_in.sigma.to(device=device, dtype=dtype) for layer in layers]
    ).reshape(-1, 1, 1)
    sigma_out_stack = torch.stack(
        [layer.factor_out.sigma.to(device=device, dtype=dtype) for layer in layers]
    ).reshape(-1, 1, 1)
    ...
    k_in = ref.factor_in.profile(sq_in, sigma_in_stack)  # [L, n_in, K]
    k_out = ref.factor_out.profile(sq_out, sigma_out_stack)  # [L, n_out, K]
