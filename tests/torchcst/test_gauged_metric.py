"""The pullback metric of L2-normalised factor columns."""
...
def _autograd_column_diagonal(factor, mu, centers):
    ...
    for center in centers:
        ...
        def delivered(value):
            return gauge.columns(factor, mu, value[None, :])[:, 0]
...
def test_normalized_diagonal_matches_the_delivered_columns_autograd(dimension):
    factor = GaussianFactor(0.3).double()
    ...
    unit, slope = metric.normalized_columns(factor, mu, centers)
    actual = metric.projected_directional(
        unit, slope, mu, centers, None, per_axis=True
    )
    expected = _autograd_column_diagonal(factor, mu, centers)
...
def test_a_far_gaussian_keeps_a_finite_positive_normalized_metric():
    ...
    raw, raw_factor = metric.columns(factor, mu, centers)
...
def test_full_atom_diagonal_factorises_over_its_two_normalized_sides():
    ...
    unit_in, slope_in = metric.normalized_columns(factor, mu_in, source)
    unit_out, slope_out = metric.normalized_columns(factor, mu_out, target)
    ...
    def represented(s, t):
        column_in = gauge.columns(factor, mu_in, s[None, :])[:, 0]
        column_out = gauge.columns(factor, mu_out, t[None, :])[:, 0]
...
def test_traffic_weights_the_already_projected_normalized_derivative():
    ...
    jac_in = torch.autograd.functional.jacobian(
        lambda s: gauge.columns(factor, mu_in, s[None, :])[:, 0], source[0]
    )
    jac_out = torch.autograd.functional.jacobian(
        lambda t: gauge.columns(factor, mu_out, t[None, :])[:, 0], target[0]
    )
    in_column = gauge.columns(factor, mu_in, source)[:, 0]
    out_column = gauge.columns(factor, mu_out, target)[:, 0]
    expected_s = (
        mass.square()
