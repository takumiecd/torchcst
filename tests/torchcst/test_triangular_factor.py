...
The triangular factor exists to connect the continuous families to the entry
...
def _store(
    ...
    *,
    ...
    store = SynapseStore(
        ...
        1,
        count,
        spec=RepresentationSpec.continuous(1, 1, factor=factor),
        dtype=torch.float64,
    )
    store.apply(
        [
...
def test_triangular_slope_is_constant_inside_the_support() -> None:
    ...
    factor(query, centers).sum().backward()
...
def test_triangular_sigma_is_learnable_and_receives_gradient() -> None:
    factor = TriangularFactor(0.5).double()
    query = torch.tensor([[0.1]], dtype=torch.float64)
    centers = torch.tensor([[0.3]], dtype=torch.float64)

    factor(query, centers).sum().backward()

    assert factor.learnable
    assert factor.sigma.grad is not None and bool(factor.sigma.grad != 0)
...
def test_gaussian_profile_is_unchanged_by_the_shared_base() -> None:
    ...
    torch.testing.assert_close(
        factor(query, centers), torch.exp(-squared / (2.0 * 0.35**2))
    )
    assert isinstance(factor, ContinuousFactor)
    assert factor.family == "gaussian" and TriangularFactor(1.0).family == "triangular"


def test_factor_validation_is_shared_with_gaussian() -> None:
    for bad in (0.0, -1.0, float("inf")):
        with pytest.raises(ValueError):
            TriangularFactor(bad)
    with pytest.raises(TypeError):
        TriangularFactor(True)
    with pytest.raises(TypeError):
        TriangularFactor(0.1, learnable="yes")  # type: ignore[arg-type]


def test_continuous_factor_base_refuses_to_be_instantiated() -> None:
    with pytest.raises(TypeError):
        ContinuousFactor(0.1)
...
def test_forward_rejects_a_sigma_that_went_bad_after_construction() -> None:
    ...
    factor = GaussianFactor(0.3, learnable=False)
    factor.sigma.copy_(torch.tensor(-1.0))
    query = torch.tensor([[0.0]])
    centers = torch.tensor([[0.0]])

    with pytest.raises(ValueError):
        factor(query, centers)
...
def test_validate_sigma_false_skips_the_per_forward_guard() -> None:
    ...
    with pytest.raises(ValueError):
        GaussianFactor(-1.0, learnable=False, validate_sigma=False)

    factor = GaussianFactor(0.3, learnable=False, validate_sigma=False)
    factor.sigma.copy_(torch.tensor(-1.0))
    query = torch.tensor([[0.0]])
    centers = torch.tensor([[0.0]])
...
def test_validate_sigma_must_be_a_bool() -> None:
    with pytest.raises(TypeError):
        GaussianFactor(0.3, validate_sigma="no")  # type: ignore[arg-type]
...
def test_spec_admits_triangular_as_a_continuous_family() -> None:
    spec = RepresentationSpec.continuous(2, 3, factor="triangular")

    assert spec.factor_in == spec.factor_out == "triangular"
    assert spec.atom_cost == 6
    assert spec.retirement == "gate_only"


def test_spec_rejects_unknown_and_mixed_factor_pairs() -> None:
    with pytest.raises(ValueError):
        RepresentationSpec.continuous(1, 1, factor="epanechnikov")
    with pytest.raises(ValueError):
        RepresentationSpec(
            domain_in=RepresentationSpec.continuous(1, 1).domain_in,
            domain_out=RepresentationSpec.continuous(1, 1).domain_out,
            factor_in="gaussian",
            factor_out="triangular",
            atom_cost=3,
            retirement="gate_only",
        )


def test_compute_module_rejects_a_factor_of_another_family() -> None:
    store = _store(
    ...
    with pytest.raises(ValueError):
        CSTLinear(inputs, outputs, store, GaussianFactor(0.3).double())
...
def test_triangular_forward_matches_the_materialized_weight() -> None:
    ...
    module = CSTLinear(inputs, outputs, store, TriangularFactor(0.3).double())
...
def test_triangular_weight_is_structurally_sparse() -> None:
    ...
    triangular = CSTLinear(inputs, outputs, store, TriangularFactor(0.15).double())
...
def test_small_sigma_reproduces_the_entry_family() -> None:
    ...
    This is why a compact factor is the only continuous profile that reaches
    ...
    store = _store(
        in_mu[columns].clone(), out_mu[rows].clone(), weights, factor="triangular"
    )
    ...
    module = CSTLinear(
        ...
        store,
        TriangularFactor(0.5 * spacing).double(),
    )
...
def test_mass_scale_tracks_the_triangular_profile() -> None:
    ...
    module = CSTLinear(inputs, outputs, store, TriangularFactor(0.2).double())

    module(torch.zeros(1, 9, dtype=torch.float64))

    mass = store.view().mass
    k_in = module.factor_in(inputs.mu, store.view().s)
    k_out = module.factor_out(outputs.mu, store.view().t)
    expected = (
        store.view().w.abs()
