...
def build_arm(
    ...
    spec = (
        RepresentationSpec.entry(
            bounds_in=IN_FEATURES, bounds_out=HIDDEN_FEATURES
        )
        if config.family == "entry"
        else RepresentationSpec.rank_one(IN_FEATURES, HIDDEN_FEATURES)
    )
    dimensions = (1, 1) if config.family == "entry" else (IN_FEATURES, HIDDEN_FEATURES)
    store = SynapseStore(
        "mnist-cst",
        *dimensions,
    ...
    cst = (
        EntryLinear(store, IN_FEATURES, HIDDEN_FEATURES)
        if config.family == "entry"
        else RankOneLinear(store, IN_FEATURES, HIDDEN_FEATURES)
    )
    ...
    audit = EconomyAudit(immunity_events=config.immunity_events)
...
def _print_table(runs: list[dict[str, object]]) -> None:
    headers = ("arm/seed", "accuracy", "params", "K_final", "thrash", "quiet")
    rows = []
    for run in runs:
        thrash = run["thrash"]
        k = run["k"]
